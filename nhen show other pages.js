...
(function () {
  ...
  function lastpage() {
    const str = document.querySelector(".last");
    const pieces = str.href.split("page=");
    const last = pieces[pieces.length - 1];

    return last;
  }
  function currentpage() {
    const str = document.querySelector(".page.current");
    const pieces = str.href.split("page=");
    const last = pieces[pieces.length - 1];
    return last;
  }
  function getlinkweb() {
    return document.querySelector(".page").href.split("page=")[0] + "page=";
  }
      ...
      onload: function (response) {
        ...
        if (sections.length < 1) {
          console.log(
            ...
            "\n" + "isi respon : " + response.responseText,
            "\n"
          );
          document.querySelector(".textTO").textContent =
            "- " + pagenumber + "error";
        } else {
          ...
          document.querySelector(".elemenInPage").textContent =
            " ▶️ " + amountElementInPage();
          ...
          // );
          reloadImage();
          console.log("berhasil load page: ", pageLoadNow++);
        }
      },
    });
  }
  ...
  document.querySelector(".btnplus").addEventListener("click", () => {
    console.log(
        "pageLoadNow: " + pageLoadNow,
        "\n" , "lastpage(): " + lastpage(),
        "\n" , "getlinkweb(): " + getlinkweb(),
        "\n"
    );
